...
class MobileVideoGPTTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if 0 and getattr(self.args, 'tune_mm_mlp_adapter', False):
            ...
            super(MobileVideoGPTTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if 0 and getattr(self.args, 'tune_mm_mlp_adapter', False):
            pass
        else:
            super(MobileVideoGPTTrainer, self)._save(output_dir, state_dict)
