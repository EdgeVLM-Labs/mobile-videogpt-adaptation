...
from mobilevideogpt.train.trainer import MobileVideoGPTTrainer
from mobilevideogpt import conversation as conversation_lib
from mobilevideogpt.model import *
from mobilevideogpt.mm_utils import tokenizer_image_token
from mobilevideogpt.config import DataConfig
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    lora_checkpoint: str = field(
        default="",
        metadata={"help": "Path to LoRA checkpoint for continued training (optional)"}
    )
        ...
        for sentence in source:
            if DEFAULT_IMAGE_TOKEN in sentence['value'] or DEFAULT_VIDEO_TOKEN in sentence['value']:
                ...
                if sentence['value'].endswith(DEFAULT_VIDEO_TOKEN):
                    ...
                    #print("VIDEO_TOKEN_NUM: ", VIDEO_TOKEN_NUM)
            ...
            if data_args.mm_use_im_start_end:
                ...
                #print("vid_replace_token: ", vid_replace_token)

            sentence["value"] = sentence["value"].replace(DEFAULT_IMAGE_TOKEN, replace_token)
            sentence['value'] = sentence['value'].replace(DEFAULT_VIDEO_TOKEN, vid_replace_token)

    #print("Preprocessed sources: ", sources)
    return sources

def preprocess_multimodal_3(sources: Sequence[str], data_args) -> Dict:
    is_multimodal = data_args.is_multimodal
    if not is_multimodal:
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            if isinstance(sources, dict):  # Case 1: sources is a dictionary
                sources = [copy.deepcopy(sources['conversations'])]
            elif isinstance(sources, list): # Case 2: sources is a list of dictionaries
        ...
        if 'image' in self.list_data_dict[i]:
            ...
            data_dict['context_image'] = torch.zeros(3, crop_size['height'], crop_size['width'])
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for i in range(len(input_ids)):
            ...
            #print("length of labels[i]: ", len(labels[i]))
            #print("length of attention_mask[i]: ", len(attention_mask[i]))

            if (input_ids[i] == IMAGE_TOKEN_INDEX).sum() == 0:
                #print("EMPTY length of input_ids[i] is: ", len(input_ids[i]))
                #print("EMPTY length of labels[i]: ", len(labels[i]))
                #print("EMPTY length of attention_mask[i]: ", len(attention_mask[i]))
...
def count_parameters(module):
    """Counts the number of parameters in a PyTorch module."""
    return sum(p.numel() for p in module.parameters())

def print_parameters_recursive(model, parent_name=""):
    """Recursively prints the total parameters for each sub-component in the model."""
    # Loop through each named sub-module
    for name, submodule in model.named_children():
        # Form the full name of the submodule (parent + current name)
        full_name = f"{parent_name}.{name}" if parent_name else name

        # Count parameters for the current submodule
        total_params = count_parameters(submodule)

        # Print the total parameters for the submodule
        print(f"Total parameters for {full_name}: {total_params}")

        # Recursively print the parameters for its submodules (if any)
        print_parameters_recursive(submodule, full_name)

def sum_parameters_at_one_level(model, level_name=""):
    """Sums the parameters for all sub-components at one level under the specified module."""
    # Find the submodule at the desired level
    submodule = dict(model.named_modules()).get(level_name)

    if submodule:
        total_sum = 0
        # Loop through each named sub-module at this level
        for name, sub in submodule.named_children():
            total_params = count_parameters(sub)
            print(f"Total parameters for {level_name}.{name}: {total_params}")
            total_sum += total_params

        print(f"\nTotal parameters for all sub-components under '{level_name}': {total_sum}")
    else:
        print(f"Module '{level_name}' not found in the model.")
...
def train():
    ...
    if model_args.vision_tower is not None:
        if 'InternVideo' in model_args.vision_tower:
            model_args.vision_tower = f"{model_args.vision_tower}/InternVideo2-stage2_1b-224p-f4.pt"
        else:
        ...
        if training_args.lora_checkpoint:
            ...
            try:
                ...
                model = PeftModel.from_pretrained(
                    model, 
                    training_args.lora_checkpoint,
                    is_trainable=True
                )
                rank0_print("✓ Existing LoRA adapters loaded successfully for continued training!")
                
            except Exception as e:
                # If loading fails, this is a critical error
                raise ValueError(
                    f"ERROR: Failed to load LoRA adapters from '{training_args.lora_checkpoint}'.\n"
                    f"Please verify:\n"
    ...
    print("Total parameters for each sub-component:")
    ...
    trainer = MobileVideoGPTTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
