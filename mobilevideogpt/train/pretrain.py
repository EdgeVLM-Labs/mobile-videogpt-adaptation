...
def preprocess_qwen(sources, tokenizer: transformers.PreTrainedTokenizer, has_image: bool = False, max_len=2048, system_message: str = "You are a helpful assistant.") -> Dict:
    ...
    #print("tokenizer.additional_special_tokens_ids : ", tokenizer.additional_special_tokens_ids)
    #im_start, im_end, image_token_index = tokenizer.additional_special_tokens_ids
...
def preprocess_plain_multiple(
    ...
    for source in sources:
        ...
        # Ensure that the source length is even
        if (len(source) % 2 == 0): #"Source must contain an even number of entries."

            image_token_found = False  # Track if the DEFAULT_IMAGE_TOKEN has been handled
            for i in range(0, len(source), 2):
                human_turn = source[i]['value']
                gpt_turn = source[i + 1]['value']
                # If it's the first turn containing an image, replace its content with DEFAULT_IMAGE_TOKEN
                if not image_token_found and DEFAULT_IMAGE_TOKEN in human_turn:
                    image_token_count = human_turn.count(DEFAULT_IMAGE_TOKEN)
                    human_turn = DEFAULT_IMAGE_TOKEN * image_token_count
                    image_token_found = True  # Mark that the image token has been handled

                # Concatenate the human and GPT turns with a separator
                conversation = human_turn + gpt_turn + conversation_lib.default_conversation.sep
                conversations.append(conversation)
...
def preprocess_plain_old(
    ...
    for source in sources:
        ...
        print("DEFAULT_IMAGE_TOKEN is: ", DEFAULT_IMAGE_TOKEN)
        print("source[0]['value'] is: ", source[0]['value'])

        assert len(source)%2 == 0
        for i in range(0,len(source),2):
            assert DEFAULT_IMAGE_TOKEN in source[i]['value']
            source[i]['value'] = DEFAULT_IMAGE_TOKEN * source[i]['value'].count(DEFAULT_IMAGE_TOKEN)
            conversation = source[i]['value'] + source[i+1]['value'] + conversation_lib.default_conversation.sep
            conversations.append(conversation)
    # tokenize conversations
    input_ids = [tokenizer_image_token(prompt, tokenizer, return_tensors='pt') for prompt in conversations]
    targets = copy.deepcopy(input_ids)
...
class LazySupervisedDataset(Dataset):
    """Dataset for supervised fne-tuning."""
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources:
            ...
            try:
                ...
                image_file, ext = os.path.splitext(image_file)
                image = Image.open(os.path.join(image_folder, image_file.replace("\\", "/"))).convert('RGB')
            ...
            #print("CURRENT SAMPLE DOES NOT HAVE IMAGE")
        ...
        if isinstance(i, int):
            if isinstance(data_dict["input_ids"], list) and len(data_dict["input_ids"]) > 0:
                data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0])
            else:
                #print("input_ids is: ", data_dict["input_ids"][0])
                #print("labels is: ", data_dict["labels"][0])
                data_dict = dict(input_ids=data_dict["input_ids"][0], labels=data_dict["labels"][0])
        #print("self.list_data_dict[i]: ", self.list_data_dict[i])
        # Image exists in the data
        if 'image' in self.list_data_dict[i]:
            data_dict['image'] = image
        elif self.data_args.is_multimodal:
            #print("MULTI MODAL")
            #print("self.list_data_dict[i] is: ", self.list_data_dict[i])
