#!/usr/bin/env python3
"""
Generate Test Evaluation Report with Confidence Scoring
...
# Import confidence thresholds from local module
...
def create_excel_report(results: List[Dict], output_path: str, use_bert: bool = True,
    ...
    ws.column_dimensions[get_column_letter(col_idx + 3)].width = 15  # Is Confident
    ws.column_dimensions[get_column_letter(col_idx + 4)].width = 12  # Avg Entropy
    ...
    for idx, result in enumerate(results, start=1):
        ...
        # Check confidence based on metrics in result
        is_confident = check_confidence_from_metrics(result)
        ...
        # Is Confident column
        confident_col_idx = col
        ws.cell(row=row, column=col).value = "TRUE" if is_confident else "FALSE"
        col += 1

        # Confidence metrics (not highlighted, just data)
        ws.cell(row=row, column=col).value = round(avg_entropy, 4)
        col += 1
        ws.cell(row=row, column=col).value = round(seq_logprob_normalized, 4)
        col += 1
        ...
        # Throughput metrics
        ws.cell(row=row, column=col).value = round(throughput, 2) if throughput > 0 else "N/A"
        col += 1
        ws.cell(row=row, column=col).value = round(gen_time, 4) if gen_time > 0 else "N/A"
        col += 1
        ws.cell(row=row, column=col).value = gen_tokens if gen_tokens > 0 else "N/A"
        col += 1
        ...
        for c in range(1, col + 1):
            ...
            if c == confident_col_idx:  # Is Confident column
                if is_confident:
                    cell.fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")
                else:
                    cell.fill = PatternFill(start_color="FFC7CE", end_color="FFC7CE", fill_type="solid")
