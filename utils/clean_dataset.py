...
- Creates a report.csv with results
"""
...
CLEANED_DATASET_PATH = Path("claned_dataset")


MIN_FRAME_WIDTH = 640
MIN_FRAME_HEIGHT = 360
MIN_SHARPNESS_SCORE = 41
MIN_BRIGHTNESS = 35
MAX_BRIGHTNESS = 190
...
def analyze_video_quality(video_path: Path, num_frames: int, frame_stride: int):
    ...
    if motion_pairs > 0:
        active_frame_pct = active_count / motion_pairs
        motion_detected = active_frame_pct >= MOTION_MIN_ACTIVE_FRAME_PCT
    ...
    metrics = {
        ...
        "motion_detected": True # Temporarily disable motion check
    }
    return metrics, issues


def evaluate_video_acceptance(metrics: dict, issues: list, stats: dict) -> bool:
    """Check if a video meets the minimum quality thresholds for acceptance."""
...
def generate_cleaning_report(overall_exercise_stats, totals, destination_root: Path = None):
    ...
    if destination_root is not None:
        report_dir = destination_root / "reports"
    ...
    for root, _, files in os.walk(source_root):
        ...
        with tqdm(total=len(files), desc=f"Processing {folder_name}", unit="video", ncols=80, leave=False) as pbar:
            for file in files:
                ...
                metrics, issues = analyze_video_quality(video_path, NUM_SAMPLED_FRAMES, FRAME_STRIDE)
                accepted = evaluate_video_acceptance(metrics, issues, exercise_stats)
